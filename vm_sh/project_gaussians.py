...
class _ProjectGaussians(Function):
    ...
    def forward(
        ...
        if num_points < 1:
            raise ValueError(
                f"No 3D points found! means3d has shape {means3d.shape}. "
                f"This usually means:\n"
                f"1. COLMAP reconstruction failed or produced no 3D points\n"
                f"2. Data path is incorrect\n"
                f"3. sparse/points3D.bin file is empty or missing\n"
                f"Please check your dataset and COLMAP output."
            )
        if means3d.shape[-1] != 3:
            raise ValueError(f"Invalid shape for means3d: {means3d.shape}, expected [..., 3]")
